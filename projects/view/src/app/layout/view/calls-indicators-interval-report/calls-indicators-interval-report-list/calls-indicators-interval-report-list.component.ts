...
export class CallsIndicatorsByIntervalReportListComponent implements OnInit {
  ...
  constructor(
    ...
    this.selectorVisibleFields.agent = false;
        ...
        .subscribe(
          (res: BackendResponseModel) => {
            ...
            if (Array.isArray(res.detail)) {
              ...
              this.temp_rows_detail_original = this.rows_original.concat(
                this.rows_subtotal
              );
  ...
  exportToExcel(data, total) {
    const filterData = data.concat(total).map((x) => {
      return {
        ...
        porcentaje_nivel_servicio_60s: (
          x.inboundServiceLevelMinute * 100
        ).toFixed(2),
  ...
  onCreateModel(model?) {
    ...
    let newModel2 = JSON.stringify(
      newModel.replace(/\"/g, "").replace(/:/g, ": ").replace(/,/g, ",\n ")
    );
